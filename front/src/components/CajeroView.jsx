import { useState, useEffect, useContext } from 'react';
import {
    Box,
    Typography,
    DialogTitle,
    DialogContent,
    DialogActions,
    Alert,
    IconButton,
    Tooltip,
    MenuItem,
    Paper,
    Chip,
    CardContent,
    CardActions,
    InputAdornment,
    useMediaQuery,
    TableContainer,
    Table,
    TableHead,
    TableRow,
    TableCell,
    TableBody
} from '@mui/material';
import { StyledCard } from '../styledComponents/ui/StyledCard';
import { StyledButton } from '../styledComponents/ui/StyledButton';
import { StyledDialog } from '../styledComponents/ui/StyledDialog';
import { StyledTextField } from '../styledComponents/ui/StyledTextField';
import Grid from '@mui/material/Unstable_Grid2';
import {
    AccountBalanceWallet as CashIcon,
    History as HistoryIcon,
    Assessment as ReportIcon,
    Close as CloseIcon,
    Visibility as ViewIcon,
    Print as PrintIcon,
    Money as MoneyIcon,
    Clear as ClearIcon,
    KeyboardArrowDown
} from '@mui/icons-material';
import { motion } from "framer-motion";
import moment from 'moment';
import 'moment/locale/es';

import { useForm } from '../hooks/useForm';
import { useSubmit } from '../hooks/useSubmit';
import { UseFetchQuery } from '../hooks/useQuery';
import { CajaManager } from '../styledComponents/CajaManager';
import { useQueryClient } from '@tanstack/react-query'; // Importar useQueryClient
import { MyFab } from '../styledComponents/MyFab';
import { variants } from '../styles/variants';
import { AuthContext } from '../context/AuthContext';
import { mostrarExito } from '../functions/mostrarExito';
import { mostrarError } from '../functions/MostrarError';
import { mostrarInfo } from '../functions/mostrarInfo';
import { mostrarCarga } from '../functions/mostrarCarga'; // Importar mostrarCarga
import Swal from 'sweetalert2'; // Importar Swal
import CloseCashSessionDialog from '../styledComponents/CloseCashSessionDialog';
import { printCashSessionReport } from '../functions/printCashSessionReport';
import { EnhancedTable } from '../styledComponents/EnhancedTable';
import CashMovementModal from '../styledComponents/CashMovementModal';
import { useTheme } from '@mui/material/styles';
import CajeroViewSkeleton from '../styledComponents/skeletons/CajeroViewSkeleton';

const CajeroView = () => {
    const theme = useTheme();
    const isMobile = useMediaQuery(theme.breakpoints.down('sm'));
    const [activeSessions, setActiveSessions] = useState([]);
    const [sessionHistory, setSessionHistory] = useState([]);
    const [selectedSession, setSelectedSession] = useState(null);
    const [closeDialogOpen, setCloseDialogOpen] = useState(false);
    const [detailsDialogOpen, setDetailsDialogOpen] = useState(false);
    const [cajaManagerOpen, setCajaManagerOpen] = useState(false);
    const [sessionDetails, setSessionDetails] = useState(null);
    const [error, setError] = useState('');
    const [visibleFab, setVisibleFab] = useState(false);
    const [cashMovementModalOpen, setCashMovementModalOpen] = useState(false);
    const [viewingSessionId, setViewingSessionId] = useState(null);

    const queryClient = useQueryClient(); // Inicializar queryClient

    const [openFilterSection, setOpenFilterSection] = useState(false);

    // Hooks de mutación
    const { mutateAsync: saveMovement, isLoading: isSavingMovement } = useSubmit();
    const { mutateAsync: initiateClosure, isLoading: isClosing } = useSubmit();

    const [filters, handleFilterChange, resetFilters] = useForm({
        status: '',
        startDate: '',
        endDate: '',
        finalDiscrepancy: ''
    });

    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(5);
    const [totalRows, setTotalRows] = useState(0);

    const { usuario, isLoading: authLoading } = useContext(AuthContext);

    // Hook para obtener la sesión activa del usuario
    const {
        data: activeSessionData,
        isLoading: activeSessionLoading,
        error: activeSessionError,
        refetch: refetchActiveSession
    } = UseFetchQuery(
        ['activeSession', usuario?.id],
        `/cash-sessions/active/${usuario?.id}`,
        !!usuario?.id
    );

    // Construcción de la URL para el historial de sesiones
    const historyParams = new URLSearchParams({
        page: page + 1,
        limit: rowsPerPage,
        user_id: usuario?.id
    });
    if (filters.status) historyParams.append('status', filters.status);
    if (filters.startDate) historyParams.append('start_date', filters.startDate);
    if (filters.endDate) historyParams.append('end_date', filters.endDate);
    if (filters.finalDiscrepancy) historyParams.append('final_discrepancy', filters.finalDiscrepancy);
    const historyUrl = `/cash-sessions/history?${historyParams.toString()}`;

    // Hook para obtener el historial de sesiones
    const {
        data: historyData,
        isLoading: historyLoading,
        error: historyError,
        refetch: refetchHistory
    } = UseFetchQuery(
        ['sessionHistory', usuario?.id, page, rowsPerPage, filters],
        historyUrl,
        !!usuario?.id
    );

    // Hook para obtener los detalles de una sesión específica (se activa bajo demanda)
    const {
        data: detailsData,
        isLoading: isFetchingDetails,
        error: detailsError,
    } = UseFetchQuery(
        ['sessionDetails', viewingSessionId],
        `/cash-sessions/${viewingSessionId}/summary`,
        !!viewingSessionId // Solo se activa cuando viewingSessionId no es nulo
    );


    useEffect(() => {
        moment.locale('es');
        const handleScroll = () => {
            if (window.scrollY > 120) setVisibleFab(true);
            else setVisibleFab(false);
        };
        window.addEventListener('scroll', handleScroll);
        return () => window.removeEventListener('scroll', handleScroll);
    }, []);

    // Efecto para actualizar el estado de las sesiones activas
    useEffect(() => {
        if (activeSessionData) {
            setActiveSessions(activeSessionData.hasActiveSession ? [activeSessionData.session] : []);
        }
    }, [activeSessionData]);

    // Efecto para actualizar el estado del historial de sesiones
    useEffect(() => {
        if (historyData) {
            setSessionHistory(Array.isArray(historyData.sessions) ? historyData.sessions : []);
            setTotalRows(historyData.pagination.total);
        }
    }, [historyData]);

    // Efecto para manejar errores de todas las consultas
    useEffect(() => {
        const sessionError = activeSessionError?.response?.data?.message || activeSessionError?.message;
        const historyErr = historyError?.response?.data?.message || historyError?.message;
        const detailErr = detailsError?.response?.data?.message || detailsError?.message;
        setError(sessionError || historyErr || detailErr || '');
    }, [activeSessionError, historyError, detailsError]);

    // Efecto que se dispara cuando llegan los datos de los detalles de la sesión
    useEffect(() => {
        if (detailsData) {
            setSessionDetails(detailsData);
            setDetailsDialogOpen(true);
        }
    }, [detailsData]);


    const handleInitiateClosure = async (declaredAmount, notes, onSuccess) => {
        if (!selectedSession) return mostrarError('No hay una sesión seleccionada.', theme);
        mostrarCarga('Procesando cierre...', theme);
        try {
            await initiateClosure({
                url: `/cash-sessions/${selectedSession.id}/initiate-close`,
                values: {
                    cashier_declared_amount: parseFloat(declaredAmount),
                    notes: notes.trim()
                }
            });
            Swal.close();
            mostrarInfo('Tu caja ha sido enviada para revisión del administrador.', theme);
            setCloseDialogOpen(false);
            setSelectedSession(null);
            refetchActiveSession();
            refetchHistory();
            queryClient.invalidateQueries(['activeCashSession', usuario.id]); // Invalidar la query de sesión activa
            if (onSuccess) onSuccess();
        } catch (error) {
            Swal.close();
            const errorMessage = error.response?.data?.message || 'Error al iniciar el cierre de caja.';
            mostrarError(errorMessage, theme);
        }
    };

    const openCloseDialog = (session) => {
        setSelectedSession(session);
        setCloseDialogOpen(true);
        setError('');
    };

    const handleViewDetails = (session) => {
        setViewingSessionId(session.id);
    };

    const formatCurrency = (amount) => new Intl.NumberFormat('es-AR', { style: 'currency', currency: 'ARS' }).format(amount || 0);
    const formatDate = (dateString) => moment(dateString).format('DD/MM/YYYY HH:mm');
    const calculateSessionDuration = (openedAt, closedAt = null) => {
        const start = moment(openedAt);
        const end = closedAt ? moment(closedAt) : moment();
        const duration = end.diff(start, 'hours', true);
        return duration > 0 ? `${duration.toFixed(1)} hs` : '0.0 hs';
    };
    const getDiscrepancyStyle = (value) => {
        const numericValue = parseFloat(value);
        if (isNaN(numericValue) || numericValue === 0) return { color: 'text.secondary' };
        return { color: numericValue > 0 ? 'success.main' : 'error.main', fontWeight: 'bold' };
    };
    const getUserName = (session) => {
        // Prioritize session data if available (for history or active session)
        const user = session?.usuario;
        if (user?.username && user?.rol?.nombre) {
            return `${user.username} (${user.rol.nombre})`;
        }
        // Fallback to the logged-in user from context
        if (usuario?.username && usuario?.rol?.nombre) {
            return `${usuario.username} (${usuario.rol.nombre})`;
        }
        return usuario?.username || 'Usuario';
    };

    const handleSaveMovement = async (movementData) => {
        try {
            await saveMovement({ url: '/cash-sessions/movement', values: movementData });
            mostrarExito('El movimiento de caja ha sido registrado correctamente.', theme);
            setCashMovementModalOpen(false);
            refetchActiveSession();
            refetchHistory();
        } catch (error) {
            const errorMessage = error.response?.data?.message || 'Error al registrar el movimiento de caja.';
            mostrarError(errorMessage, theme);
            console.error('Error al registrar movimiento:', error);
        }
    };

    const historyColumns = [
        { id: 'opened_at', label: 'Apertura', valueGetter: ({ row }) => formatDate(row.opened_at) },
        { id: 'verified_at', label: 'Cierre', valueGetter: ({ row }) => row.verified_at ? formatDate(row.verified_at) : '-' },
        { id: 'total_sales_at_close', label: 'Ventas (Cierre)', valueGetter: ({ row }) => formatCurrency(row.total_sales_at_close || 0) },
        {
            id: 'preliminary_discrepancy',
            label: 'Dif. Preliminar',
            valueGetter: ({ row }) => formatCurrency(row.preliminary_discrepancy),
            cellStyle: (row) => getDiscrepancyStyle(row.preliminary_discrepancy)
        },
        {
            id: 'final_discrepancy',
            label: 'Dif. Final',
            valueGetter: ({ row }) => formatCurrency(row.final_discrepancy),
            cellStyle: (row) => getDiscrepancyStyle(row.final_discrepancy)
        },
        {
            id: 'status',
            label: 'Estado',
            valueGetter: ({ row }) => (
                <Chip
                    label={row.status.replace('_', ' ')}
                    color={row.status === 'cerrada' ? 'default' : row.status === 'pendiente_cierre' ? 'warning' : 'success'}
                    size="small"
                    sx={{ textTransform: 'capitalize' }}
                />
            )
        },
        {
            id: 'actions',
            label: 'Acciones',
            valueGetter: ({ row }) => (
                <>
                    <Tooltip title="Ver Detalles">
                        <IconButton size="small" onClick={() => handleViewDetails(row)}><ViewIcon /></IconButton>
                    </Tooltip>
                    <Tooltip title="Imprimir reporte">
                        <IconButton size="small" onClick={() => printCashSessionReport(row, { formatCurrency, formatDate, getUserName })}>
                            <PrintIcon />
                        </IconButton>
                    </Tooltip>
                </>
            )
        },
    ];

    const isLoading = authLoading || activeSessionLoading || historyLoading;

    if (isLoading) {
        return <CajeroViewSkeleton />;
    }

    return (
        <motion.div initial="hidden" animate="enter" exit="exit" variants={variants} transition={{ duration: 0.8, type: "easeInOut" }}>
            <Box sx={{ p: 1 }}>
                <MyFab visibleFab={visibleFab} screenSize={{ width: window.innerWidth }} />

                <Paper
                    elevation={3}
                    sx={{
                        p: { xs: 2, sm: 3 },
                        mb: 3,
                        mt: 2,
                        background: (theme) => theme.palette.background.componentHeaderBackground,
                        color: theme.palette.primary.contrastText
                    }}
                >
                    <Grid container justifyContent="space-between" alignItems="center">
                        <Grid>
                            <Typography variant={isMobile ? "h5" : "h4"} gutterBottom>
                                Mi Caja
                            </Typography>
                            <Typography variant="body1" sx={{ opacity: 0.9 }}>Gestión personal de sesiones de caja - {usuario?.username}</Typography>
                        </Grid>
                        <Grid>
                            {!activeSessions.some(s => s.status === 'abierta') && (
                                <StyledButton variant="outlined" size="large" onClick={() => setCajaManagerOpen(true)}>
                                    Abrir Caja
                                </StyledButton>
                            )}
                            {activeSessions.some(s => s.status === 'abierta') && (
                                <StyledButton
                                    variant="outlined"
                                    size="large"
                                    onClick={() => setCashMovementModalOpen(true)}
                                    startIcon={<MoneyIcon />}
                                    sx={{ ml: 2 }}
                                >
                                    Registrar Movimiento
                                </StyledButton>
                            )}
                        </Grid>
                    </Grid>
                </Paper>

                {error && <Alert severity="error" sx={{ mb: 2 }} onClose={() => setError('')}>{error}</Alert>}

                <Grid container spacing={3} mb={4} justifyContent="center">
                    <Grid xs={4}>
                        <StyledCard>
                            <CardContent>
                                <Box display="flex" alignItems="center" mb={1}>
                                    <CashIcon color="info" sx={{ mr: 1.5, fontSize: 'clamp(1.5rem, 3vw, 4rem)' }} />
                                    <Typography variant="h6" color={theme.palette.text.titleSecondary}>Mi Sesión Activa</Typography>
                                </Box>
                                <Typography variant="h5" color={activeSessions.length > 0 ? "success.main" : "error.main"} fontWeight="bold">
                                    {activeSessions.length > 0 ? 'ABIERTA' : 'CERRADA'}
                                </Typography>
                            </CardContent>
                        </StyledCard>
                    </Grid>
                    <Grid xs={4}>
                        <StyledCard>
                            <CardContent>
                                <Box display="flex" alignItems="center" mb={1}>
                                    <ReportIcon color="info" sx={{ mr: 1.5, fontSize: 'clamp(1.5rem, 3vw, 4rem)' }} />
                                    <Typography variant="h6" color={theme.palette.text.titleSecondary}>Monto en Caja</Typography>
                                </Box>
                                <Typography variant="h5" color="success.main" fontWeight="bold">
                                    {formatCurrency(activeSessions.reduce((sum, s) => sum + parseFloat(s.current_cash || 0), 0))}
                                </Typography>
                            </CardContent>
                        </StyledCard>
                    </Grid>
                    <Grid xs={4}>
                        <StyledCard>
                            <CardContent>
                                <Box display="flex" alignItems="center" mb={1}>
                                    <HistoryIcon color="info" sx={{ mr: 1.5, fontSize: 'clamp(1.5rem, 3vw, 4rem)' }} />
                                    <Typography variant="h6" color={theme.palette.text.titleSecondary}>Sesiones Hoy</Typography>
                                </Box>
                                <Typography variant="h5" color="info.main" fontWeight="bold">
                                    {sessionHistory.filter(s => moment(s.opened_at).isSame(moment(), 'day')).length}
                                </Typography>
                            </CardContent>
                        </StyledCard>
                    </Grid>
                </Grid>

                <Paper sx={{ p: 2, mb: 3 }}>
                    <Typography variant="h6" gutterBottom color={theme.palette.text.titlePrimary}>Mi Sesión Activa</Typography>
                    {activeSessions.length > 0 ? (
                        <Grid container spacing={1} justifyContent="center">
                            {activeSessions.map((session) => (
                                <Grid xs={12} sm={8} md={6} key={session.id}>
                                    <StyledCard variant="outlined">
                                        <CardContent sx={{ p: 2 }}>
                                            <Box display="flex" justifyContent="space-between" alignItems="center" mb={1.5}>
                                                <Typography variant="h6" color={theme.palette.text.titlePrimary}>{getUserName(session)}</Typography>
                                                <Chip label={session.status.replace('_', ' ')} color="success" size="small" sx={{ textTransform: 'capitalize' }} />
                                            </Box>
                                            <Typography variant="body2" color="textSecondary">Apertura: {formatDate(session.opened_at)}</Typography>
                                            <Typography variant="body2" color="textSecondary">Duración: {calculateSessionDuration(session.opened_at)}</Typography>
                                            <Typography variant="body1" sx={{ mt: 1.5, fontWeight: 'bold' }}>Monto inicial: {formatCurrency(session.opening_amount)}</Typography>
                                        </CardContent>
                                        <CardActions sx={{ p: 1.5, justifyContent: 'flex-end' }}>
                                            <StyledButton size="small" color="error" onClick={() => openCloseDialog(session)} startIcon={<CloseIcon />}>Cerrar Caja</StyledButton>
                                            <Tooltip title="Ver detalles de la sesión actual">
                                                <IconButton size="small" onClick={() => handleViewDetails(session)}><ViewIcon /></IconButton>
                                            </Tooltip>
                                        </CardActions>
                                    </StyledCard>
                                </Grid>
                            ))}
                        </Grid>
                    ) : (
                        <Typography color="textSecondary" sx={{ p: 2, textAlign: 'center' }}>No tienes ninguna sesión de caja activa.</Typography>
                    )}
                </Paper>

                <StyledCard sx={{ p: 2, mb: 3 }}>
                    <Grid container justifyContent="space-between" alignItems="center" sx={{ mb: 1 }}>
                        <Typography variant="h6">Filtros de Historial</Typography>
                        <IconButton disableRipple size="small" onClick={() => setOpenFilterSection(!openFilterSection)} sx={{ p: 0 }}>
                            <KeyboardArrowDown sx={{ transition: '0.5s', transform: openFilterSection ? 'rotate(-180deg)' : 'rotate(0)', backgroundColor: 'primary.main', color: 'primary.contrastText', borderRadius: '50%' }} />
                        </IconButton>
                    </Grid>
                    <Box sx={{ height: openFilterSection ? 'auto' : 0, overflow: 'hidden', transition: 'height 0.3s ease-in-out' }}>
                        <Grid container spacing={2} justifyContent="center" alignItems="center">

                            {/* Columna para filtros de Estado y Diferencia Final */}
                            <Grid sx={{ width: 'clamp(250px, 30%, 350px)' }}>
                                <Grid container direction="column" spacing={2}>
                                    <Grid >
                                        <StyledTextField
                                            select
                                            label="Estado"
                                            name="status"
                                            value={filters.status || ''}
                                            onChange={(e) => { handleFilterChange(e); setPage(0); }}
                                        >
                                            <MenuItem value="">Todos</MenuItem>
                                            <MenuItem value="cerrada">Cerrada</MenuItem>
                                            <MenuItem value="pendiente_cierre">Pendiente Cierre</MenuItem>
                                        </StyledTextField>
                                    </Grid>
                                    <Grid >
                                        <StyledTextField
                                            select
                                            label="Diferencia Final"
                                            name="finalDiscrepancy"
                                            value={filters.finalDiscrepancy || ''}
                                            onChange={(e) => { handleFilterChange(e); setPage(0); }}
                                        >
                                            <MenuItem value="">Todos</MenuItem>
                                            <MenuItem value="positiva">Positiva</MenuItem>
                                            <MenuItem value="negativa">Negativa</MenuItem>
                                            <MenuItem value="nula">Nula (0.00)</MenuItem>
                                        </StyledTextField>
                                    </Grid>
                                </Grid>
                            </Grid>

                            {/* Columna para filtros de Fecha Desde y Fecha Hasta */}
                            <Grid sx={{ width: 'clamp(250px, 30%, 350px)' }}>
                                <Grid container direction="column" spacing={2}>
                                    <Grid >
                                        <StyledTextField
                                            label="Fecha Desde"
                                            type="date"
                                            name="startDate"
                                            value={filters.startDate || ''}
                                            onChange={(e) => { handleFilterChange(e); setPage(0); }}
                                            InputLabelProps={{ shrink: true }}
                                            InputProps={{
                                                startAdornment: (
                                                    <InputAdornment position="start">
                                                        <IconButton size='small' onClick={() => { handleFilterChange({ target: { name: 'startDate', value: '' } }); setPage(0); }}>
                                                            <ClearIcon fontSize='small' color='error' />
                                                        </IconButton>
                                                    </InputAdornment>
                                                )
                                            }}
                                        />
                                    </Grid>
                                    <Grid >
                                        <StyledTextField
                                            label="Fecha Hasta"
                                            type="date"
                                            name="endDate"
                                            value={filters.endDate || ''}
                                            onChange={(e) => { handleFilterChange(e); setPage(0); }}
                                            InputLabelProps={{ shrink: true }}
                                            InputProps={{
                                                startAdornment: (
                                                    <InputAdornment position="start">
                                                        <IconButton size='small' onClick={() => { handleFilterChange({ target: { name: 'endDate', value: '' } }); setPage(0); }}>
                                                            <ClearIcon fontSize='small' color='error' />
                                                        </IconButton>
                                                    </InputAdornment>
                                                )
                                            }}
                                        />
                                    </Grid>
                                </Grid>
                            </Grid>

                            {/* Columna para el botón Limpiar */}
                            <Grid sx={{ width: 'clamp(150px, 15%, 200px)' }}>
                                <Grid container spacing={2} sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', height: '100%' }}>
                                    <Grid >
                                        <StyledButton variant="outlined" color="secondary" onClick={() => {
                                            resetFilters();
                                            setPage(0);
                                        }}>Limpiar</StyledButton>
                                    </Grid>
                                </Grid>
                            </Grid>
                        </Grid>
                    </Box>
                </StyledCard>

                <StyledCard sx={{ p: 2, mb: 3 }}>
                    <Typography variant="h6" gutterBottom color={theme.palette.text.titlePrimary}>Mi Historial de Sesiones</Typography>
                    <EnhancedTable
                        columns={historyColumns}
                        data={sessionHistory}
                        loading={authLoading || activeSessionLoading || historyLoading}
                        count={totalRows}
                        page={page}
                        rowsPerPage={rowsPerPage}
                        onPageChange={(event, newPage) => setPage(newPage)}
                        onRowsPerPageChange={(event) => {
                            setRowsPerPage(parseInt(event.target.value, 10));
                            setPage(0);
                        }}
                    />
                </StyledCard>

                <CajaManager open={cajaManagerOpen} onClose={() => { setCajaManagerOpen(false); refetchActiveSession(); refetchHistory(); }} userId={usuario?.id} activeSession={activeSessions.length > 0 ? activeSessions[0] : null} />
                <CloseCashSessionDialog open={closeDialogOpen} onClose={() => setCloseDialogOpen(false)} selectedSession={selectedSession} handleInitiateClosure={handleInitiateClosure} getUserName={getUserName} isLoading={isClosing} />

                <CashMovementModal
                    open={cashMovementModalOpen}
                    onClose={() => setCashMovementModalOpen(false)}
                    onSave={handleSaveMovement}
                    activeSessionId={activeSessions.length > 0 ? activeSessions[0].id : null}
                    isSaving={isSavingMovement}
                    theme={theme} // Pass the theme prop
                />

                <StyledDialog open={detailsDialogOpen} onClose={() => {
                    setDetailsDialogOpen(false);
                    setViewingSessionId(null); // Limpiar el ID al cerrar para permitir futuras selecciones
                }} maxWidth="md" fullWidth>
                    <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'background.dialog', color: 'text.primary' }}>
                        Detalles de Mi Sesión
                        <IconButton onClick={() => { setDetailsDialogOpen(false); setViewingSessionId(null); }}>
                            <CloseIcon color="error" />
                        </IconButton>
                    </DialogTitle>
                    <DialogContent sx={{ backgroundColor: 'background.dialog', p: 3 }}>
                        {isFetchingDetails ? (
                            <Typography sx={{ p: 3, textAlign: 'center' }}>Cargando detalles...</Typography>
                        ) : sessionDetails && (
                            <Box sx={{ mt: 2 }}>
                                <Grid container spacing={3}>
                                    <Grid item xs={12} md={6}>
                                        <Typography variant="h6" gutterBottom>Información General</Typography>
                                        <Box mb={1}><Typography variant="body2" color="textSecondary">Apertura:</Typography><Typography variant="body1">{formatDate(sessionDetails.opened_at)}</Typography></Box>
                                        <Box mb={1}><Typography variant="body2" color="textSecondary">Cierre:</Typography><Typography variant="body1">{sessionDetails.verified_at ? formatDate(sessionDetails.verified_at) : 'Sesión Activa'}</Typography></Box>
                                        <Box mb={1}><Typography variant="body2" color="textSecondary">Duración:</Typography><Typography variant="body1">{calculateSessionDuration(sessionDetails.opened_at, sessionDetails.closed_at)}</Typography></Box>
                                    </Grid>
                                    <Grid item xs={12} md={6}>
                                        <Typography variant="h6" gutterBottom>Resumen Financiero</Typography>
                                        <Box mb={1}><Typography variant="body2" color="textSecondary">Monto Apertura:</Typography><Typography variant="h6" color="primary">{formatCurrency(sessionDetails.opening_amount)}</Typography></Box>
                                        <Box mb={1}><Typography variant="body2" color="textSecondary">Total Ventas:</Typography><Typography variant="h6" color="success.main">{formatCurrency(sessionDetails.total_sales || 0)}</Typography></Box>
                                        {sessionDetails.payment_methods && Object.entries(sessionDetails.payment_methods).map(([method, amount]) => (
                                            <Box key={method} sx={{ mb: 0.5, pl: 2 }}>
                                                <Typography variant="body2" color="textSecondary" component="div" sx={{ display: 'flex', justifyContent: 'space-between' }}>
                                                    <span>{method}:</span>
                                                    <span>{formatCurrency(amount)}</span>
                                                </Typography>
                                            </Box>
                                        ))}
                                        {sessionDetails.closing_amount && <Box mb={1}><Typography variant="body2" color="textSecondary">Monto Cierre:</Typography><Typography variant="h6" color="info.main">{formatCurrency(sessionDetails.closing_amount)}</Typography></Box>}
                                    </Grid>
                                    <Grid item xs={12}>
                                        <Typography variant="h6" gutterBottom sx={{ mt: 2 }}>Movimientos de Caja</Typography>
                                        {sessionDetails.movements && sessionDetails.movements.length > 0 ? (
                                            <TableContainer component={Paper} sx={{ background: 'transparent', border: `1px solid ${theme.palette.divider}` }}>
                                                <Table size="small">
                                                    <TableHead>
                                                        <TableRow>
                                                            <TableCell>Tipo</TableCell>
                                                            <TableCell>Monto</TableCell>
                                                            <TableCell>Descripción</TableCell>
                                                            <TableCell>Fecha</TableCell>
                                                        </TableRow>
                                                    </TableHead>
                                                    <TableBody>
                                                        {sessionDetails.movements.map((movement, index) => (
                                                            <TableRow key={index}>
                                                                <TableCell>
                                                                    <Chip
                                                                        label={movement.type === 'ingreso' ? 'Ingreso' : 'Egreso'}
                                                                        color={movement.type === 'ingreso' ? 'success' : 'error'}
                                                                        size="small"
                                                                    />
                                                                </TableCell>
                                                                <TableCell>{formatCurrency(movement.amount)}</TableCell>
                                                                <TableCell>{movement.description}</TableCell>
                                                                <TableCell>{formatDate(movement.createdAt)}</TableCell>
                                                            </TableRow>
                                                        ))}
                                                    </TableBody>
                                                </Table>
                                            </TableContainer>
                                        ) : (
                                            <Typography variant="body2" color="text.secondary">No hay movimientos registrados para esta sesión.</Typography>
                                        )}
                                    </Grid>
                                </Grid>
                            </Box>
                        )}
                    </DialogContent>
                    <DialogActions sx={{ p: 2, backgroundColor: 'background.dialog' }}>
                        <StyledButton sx={{ padding: '2px 12px' }} variant="outlined" color="secondary" onClick={() => {
                            setDetailsDialogOpen(false);
                            setViewingSessionId(null); // Limpiar el ID al cerrar
                        }}>Cerrar</StyledButton>
                        {sessionDetails && <StyledButton onClick={() => printCashSessionReport(sessionDetails, { formatCurrency, formatDate, getUserName })} variant="contained" startIcon={<PrintIcon />}>Imprimir</StyledButton>}
                    </DialogActions>
                </StyledDialog>
            </Box>
        </motion.div>
    );
};

export default CajeroView;
