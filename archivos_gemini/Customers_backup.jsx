...
const Customers = () => {
    ...
    // Estados del formulario (mantener aquí ya que useForm es un hook separado)
    const [formData, handleInputChange, resetForm, resetArray, setFormData] = useForm();
    ...
    // Sincronizar singleCustomerData con formData para edición/visualización
    useEffect(() => {
        if (singleCustomerData?.success && (dialogMode === 'edit' || dialogMode === 'view')) {
            setFormData({
                name: singleCustomerData.data?.name || '',
                email: singleCustomerData.data?.email || '',
                phone: singleCustomerData.data?.phone || '',
                address: singleCustomerData.data?.address || '',
                dni: singleCustomerData.data?.dni || '',
                discount_percentage: singleCustomerData.data?.discount_percentage || 0,
                credit_limit: singleCustomerData.data?.credit_limit || 0,
                debt: singleCustomerData.data?.debt || 0
            });
        } else if (dialogMode === 'create') {
            // Asegurarse de que el formulario esté vacío en modo creación
            setFormData({
                name: '',
                email: '',
                phone: '',
    ...
    }, [singleCustomerData, dialogMode, setFormData]);
                    ...
                    {tienePermiso('accion_editar_cliente') && ( // Changed permission
                        <Tooltip title="Editar">
                            <IconButton
                                ...
                                onClick={() => {
                                    handleEdit(row);
                                    setFormData(row); // Cargar datos del cliente en el formulario
                                }}
                                color="primary"
                            >
    ...
    ], [handleView, handleEdit, setFormData, setSelectedCustomer, setOpenPaymentDialog, setOpenHistoryDialog, handleDelete, isMobile, tienePermiso]);
                    ...
                    {tienePermiso('accion_editar_cliente') && ( // Changed permission
                        <Tooltip title="Editar">
                            <IconButton
                                ...
                                onClick={() => {
                                    handleEdit(customer);
                                    setFormData(customer); // Cargar datos del cliente en el formulario
                                }}
                                color="primary"
                            >
                                <EditIcon fontSize="small" />
    ...
    }, [customers, handleView, handleEdit, setFormData, setSelectedCustomer, setOpenPaymentDialog, setOpenHistoryDialog, handleDelete, isMobile, tienePermiso]);
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                        ...
                        {tienePermiso('accion_crear_cliente') && (
                            <StyledButton
                                variant="outlined"
                                startIcon={<AddIcon />}
                                onClick={() => handleCreate(setFormData)}
